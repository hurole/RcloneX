...
首先确保你的系统安装了 **Node.js (v24.x)** 和 现代包管理器 **pnpm**。
...
在对代码进行本地开发或打包提交前，可通过 `pnpm` 运行以下脚本：

| 运行指令               | 命令动作描述                                | 备注                                  |
| :--------------------- | :------------------------------------------ | :------------------------------------ |
| **`pnpm run dev`**     | 启动开发服务器（支持 HMR）                  | 默认开启端口 3000 并拉起浏览器        |
| **`pnpm run lint`**    | 运行 **`oxlint`** 进行代码静态语法扫描      | 扫描速度极快，确保无低级代码错误      |
| **`pnpm run fmt`**     | 运行 **`oxfmt`** 进行全站代码格式化排版     | 采用 `@fka/oxfmt-config` 最佳实践配置 |
| **`pnpm run check`**   | 运行 **`tsc`** 进行 TypeScript 类型安全校验 | 检验全站没有任何编译级别类型报错      |
| **`pnpm run test`**    | 运行 **`Vitest`** 进行单元与集成测试        | 执行全站断言，验证无任何回归 Bug      |
| **`pnpm run build`**   | 执行生产环境编译打包                        | 打包出经过高度优化的静态文件          |
| **`pnpm run preview`** | 本地预览生产构建打包产物                    | 检验生产打包产物在本地是否运行正常    |
